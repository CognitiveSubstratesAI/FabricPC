...
function perturb_z_diag!(state, structure, clamps, rng, eps)
    for name in structure.node_names
        ...
        state = FabricPC.update_node_in_state(
            state, name;
            z_latent=ns.z_latent .+ Float32(eps) .* randn(rng, Float32, size(ns.z_latent))
        )
    end
    return state
end
...
function joint_dist_diag(stateA, stateB, velA, velB, structure, clamps)
    ...
    for name in structure.node_names
        ...
        d += sum(
            abs2,
            Float64.(stateA.nodes[name].z_latent) .- Float64.(stateB.nodes[name].z_latent)
        )
...
function perturb_and_track_renorm(
    inf::InferenceSGDMomentum; burn_in=40, n_iters=400, renorm_every=10, eps=1.0f-4, seed=7
)
    structure = build_graph(inf)
    state0 = fresh_init(structure)
    vel0 = Dict{String, Any}(
        name => zero(state0.nodes[name].z_latent) for name in structure.node_names
    )
    state, vel = state0, vel0
    for _ in 1:burn_in
        state, vel = FabricPC.momentum_inference_step(
            inf, PARAMS, state, vel, CLAMPS, structure
        )
    end
    ...
    for step in 1:n_iters
        stateA, velA = FabricPC.momentum_inference_step(
            inf, PARAMS, stateA, velA, CLAMPS, structure
        )
        stateB, velB = FabricPC.momentum_inference_step(
            inf, PARAMS, stateB, velB, CLAMPS, structure
        )
...
@testset "InferenceSGDMomentum spectrum diagnostic (perturb-and-track, corrected)" begin
    ...
    lr_ctrl = perturb_and_track_renorm(
        InferenceSGDMomentum(;
            eta_infer=0.01f0, infer_steps=1, latent_decay=0.0, momentum=0.0f0
        )
    )
    ctrl_rate = exp(sum(lr_ctrl[(end - 19):end]) / 20)
    println(
        "CONTROL (momentum=0, eta=0.01) converged rate: $(ctrl_rate)  (original docs/decisions.md #24 claim: ~0.977 -- an UNDER-converged short-iteration estimate)"
    )
    ...
    lr_opt = perturb_and_track_renorm(
        InferenceSGDMomentum(;
            eta_infer=Float32(eta_star), infer_steps=1, latent_decay=0.0,
            momentum=Float32(beta_star)
        )
    )
    opt_rate = exp(sum(lr_opt[(end - 19):end]) / 20)
    println(
        "MOMENTUM corrected-optimal (eta*=$(eta_star), beta*=$(beta_star)) converged rate: $(opt_rate)"
    )
    println(
        "  naive quadratic-model prediction sqrt(beta*) = $(sqrt(beta_star)) -- NOT expected to match closely (trajectory-dependent Hessian, see test_inference_momentum.jl's file header)"
    )
    println(
        "  what IS the claim: momentum measurably faster than plain SGD's own (corrected) rate, same methodology"
    )
