...
# 🔴 TEST-ONLY DEPS NEED THE TEST ENVIRONMENT, and `--project=.` does NOT provide them.
# A package whose Project.toml has `[targets] test = ["Test", "Zygote", ...]` keeps those OUT of the
# main manifest; `Pkg.test` builds a temporary env for them. Running `include(test/runtests.jl)`
# under `--project=.` therefore dies with "Package Zygote not found in current path" — which reads
# like a broken package and is actually a missing environment. MEASURED on FabricPC 2026-08-21:
# NNlib, then Zygote, all four test deps present in the DEPOT but absent from the env.
# ⇒ TestEnv.activate() adds exactly the [targets] deps, then we include as normal. It is a no-op for
#   packages whose only test dep is Test (MORK, PathMap), so the same runner serves both shapes.
TESTENV_LINE=""
# A comma in the test target IS the signal: `test = ["Test"]` has none, `test = ["Test","Zygote"]`
# does. (An earlier version used a PCRE lookahead, which grep -E does not support — it would have
# failed silently to /dev/null and never activated TestEnv.)
if grep -A2 '^\[targets\]' "$ROOT/Project.toml" 2>/dev/null | grep -q 'test *=.*,'; then
  TESTENV_LINE='try; @eval using TestEnv; TestEnv.activate(); catch e; @warn "TestEnv unavailable — test-only deps may be missing" e; end'
fi
