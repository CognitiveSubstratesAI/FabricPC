...
**Corrected claim 1 — the in-place path is FLOP/bandwidth-LIMITED, within ~6% of its physical floor (NOT "≈ the FLOP ceiling").** Decompose stock `= F + O`: from `153.3 = F+O` and `29.6 = F/26.82 + O` ⟹ `F ≈ 128.5 ms, O ≈ 24.8 ms`; arithmetic floor `F/26.82 ≈ 4.79 ms`; measured in-place **5.1 ms** = 6% above. Independently from op shapes: one hoisted 51.4-MFLOP contraction + 40 skinny memory-bound 0.66-MFLOP dots ≈ same order. Two routes, ~6% agreement — there is no software overhead left to remove.
...
**Corrected claim 3 — the strategy is now MEASURED, not inferred from a coincidence.** The decisive number: `T_reactant`, Julia+Reactant's absolute time on this exact workload — the SAME 77.6 MFLOP computation XLA also hoist+DCE'd, directly comparable to the in-place eager time (numerics agree to 3.2e-6). Measured: **`T_reactant` = 5.75 ms** (XLA, its own ~2-thread pool) vs **in-place eager 4.49 ms (BLAS=2) / 5.1 ms (BLAS=1)**. **Optimized eager Julia MATCHES-to-BEATS XLA for this workload** (~1.28× faster on matched threading; still faster even crippled to BLAS=1). So the compiled/Reactant lane's value is empirically NOT raw eager speed — it is (a) SCALING (`@trace while` for the ~89–2300 steps §24/§26 showed the transformer needs, which an eager loop pays linearly per step) and (b) cross-language/GPU parity (J-04's 7.87× was vs `jax.jit`'s slow ~42 ms — and optimized eager Julia at ~5 ms would beat that too). This reframes §11's ladder: "compiled removes the same boxing/Dict overhead a hand-optimized eager path removes manually, and for small step-counts eager wins outright."
