...
**XLA's per-step compute is ~15–20% FASTER than optimized eager Julia** (0.127 vs 0.148–0.154). Eager's *total* only won at 20 steps because XLA's ~2.5 ms/call marshalling (a naive-usage cost — a real deployment marshals once, not per call) amortizes poorly over few steps; crossover is **~39 steps**, above which XLA wins outright (N=80: 12.7 vs 13.8 ms). So the earlier "compiled ≠ raw speed" conclusion is WRONG — XLA is faster on compute even here.

**And this is the topology MOST favorable to eager, so the general claim was doubly unsafe.** MNIST-MLP's 98.7% FLOP elimination (§28) leaves XLA only 40 tiny N=10 dots — a poor fusion surface where its per-kernel overhead is worst. On the transformer, elimination is EXACTLY 0% (§28 table), so the residual is 100% of a large, highly-fusible MHA+FFN computation (layernorm→GEMM→softmax→GEMM), precisely where XLA fusion earns its keep and eager must materialize every intermediate — the per-step gap should WIDEN and marshalling amortize to nothing. The result very likely reverses further against eager there. It is also not currently runnable (no `_flat_input_grads(::TransformerBlock)` ⇒ no compiled transformer inference), so the general "compiled = scaling not speed" statement was untestable when first written. **Scoped conclusion: for a shallow MLP at low step-counts, optimized eager Julia is competitive on total time via zero marshalling; XLA is faster per-step everywhere measured, and the compiled lane's raw-speed advantage should GROW with step-count and fusible topology — measure on `transformer_lm()` before any roadmap claim.** (J-04's 7.87× vs `jax.jit`'s ~42 ms is a separate, still-valid cross-*framework* result — XLA-vs-XLA-via-two-frontends, not XLA-vs-eager.)
