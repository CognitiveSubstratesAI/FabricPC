...
| C-14 | MED | Enzyme lane: hoist `SoA(inputs)` out of `_ad_param_grads`'s closure (uniform with the Zygote lane) | **VERIFIED CLOSED — AS UNNECESSARY (2026-07-15). The trigger fired on schedule and the answer was "no change needed", which is a RESULT, not a reprieve.** ConvNode landed ⇒ tested immediately (`test/dual_ad_backend_env`, Enzyme without Zygote). The Enzyme lane produces correct gradients on ConvNode both single-edge (dE/dW 1.99e-7, dE/db 1.16e-7, dE/dx 1.95e-7, dE/dz 7.36e-8 vs the tier_conv oracle) AND **3-in-edge — the first seam node that ITERATES `inputs` on the tape** (all three dE/dW 8.93e-8–1.51e-7, all three dE/dx 9.7e-8–1.32e-7, dE/dz 8.13e-8). **The predicted hazard did not materialise**: `Const(inputs)` means Enzyme never differentiates through the Dict, so the i64-hash-internals problem `core/types.jl:47-54` documents (which is real for a DIFFERENTIATED Dict) simply does not arise for a constant one. The prescribed SoA hoist is therefore unnecessary for correctness — it would have been uniformity-for-its-own-sake. The restraint in NOT making that edit blind (unverifiable in `.warm/zygote_env`, which has no Enzyme) was correct, and is now verified-correct rather than luckily-correct. Net: both AD backends independently agree with upstream on the same fixtures. **The trigger design is the transferable part:** ConvNode simultaneously made the hazard live AND testable, so the deferral's own precondition expired the moment the work landed — a note saying "don't forget" would have decayed; a trigger keyed to a specific event could not. ~~DEFERRED — WITH AN EXPLICIT TRIGGER~~ Why it is *currently* correct to defer: `ext/FabricPCEnzymeExt.jl:58-62` passes `Const(inputs)` — Enzyme does not differentiate `Const` args, so `df1d470`'s `Dict`→`OrderedDict` change (B3) is genuinely INERT there, and `.warm/zygote_env` has no Enzyme, so touching it would be an unverifiable blind edit (`feedback_no_coldstart_testing` / don't change what you cannot run). Why the trigger is exactly ConvNode: the Zygote lane's identical bug was dormant *only* because every seam node is single-edge; **ConvNode/MaxPool/AvgPool are the first `is_multi_input` seam nodes and the first to ITERATE `inputs` on the tape** — and `src/core/types.jl:47-54` records that a `Dict`'s i64 hash internals are what trips Enzyme's `addToDiffe`. So ConvNode simultaneously (a) makes the hazard live and (b) makes it testable. Per the C-01 plan: if the Enzyme lane aborts on ConvNode, the Zygote lane still closes C-01 and the Enzyme gap is recorded HERE as a gap — it does not get absorbed into a conformance claim. |
