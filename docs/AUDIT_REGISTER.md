...
| ID | Pair | Status |
|----|------|--------|
| R-01 | train_autoregressive.jl vs .py (full) | **PARTIALLY CLOSED — do not read as a full-file review.** What's actually verified, each for a SPECIFIC named sub-question, not a systematic pass: formatting-safety (Blue-reformat was whitespace-only except one defect, `ecb031a`); the one-hot/clamp contract (F-05, this session); `_eval_step_autoregressive`/`evaluate_autoregressive` ported faithfully (C-06, this session); the generation index-cast question (this session — read `_generation_step`/`generate_autoregressive`, train_autoregressive.py:334-538, directly: the scan carry is int32 end-to-end for the EmbeddingNode path, matching Julia's native-`Int` loop-carried state; the only float involvement is X-01's already-documented transient per-step clamp cast, bit-exact and non-compounding — same divergence X-01 already covers, not a new one). What's NOT been done: a genuine line-by-line comparative pass over the ENTIRE file looking for undiscovered divergences beyond these four named questions. One already-known gap that's never been promoted to a tracked register item: `train_autoregressive`'s own docstring notes "fractional epochs and gradient-accumulation (an upstream read-but-unused stub) are not ported — integer epochs" — a real, acknowledged divergence sitting in a comment, not in this register. A real "R-01 in full" pass should (a) turn that into a proper X-item or confirm it's inert (upstream's own stub is unused, per the docstring — verify that claim directly rather than trust the comment), and (b) systematically diff every remaining function pair not yet covered by the four bullets above. Coverage caveat for generation specifically: `test_transformer_lm.jl`'s "(c) greedy generation" testset is a self-consistency smoke test (shape/prefix/range/RNG-determinism), not a byte-level fixture against upstream JAX output like Tier A/B. |
...
§21-23 written). R-02…R-05 closed — R-05's graph_construction spot-check found and fixed a real
(dormant) duplicate-edge double-counting bug in the JIT flat path. **R-01 is PARTIALLY closed
only** — its four named sub-questions are verified, but a genuine full-file comparative pass is
still open, including one already-acknowledged-in-a-comment-but-never-tracked gap
(fractional-epoch/gradient-accumulation, see R-01's own row). C-04 (reclassified, not blocking)
remains as its inputs arrive.
