...
| C-04 | HIGH | PC-vs-backprop comparison harness | **DECIDE → OPEN, reclassified.** Reflective cross-check confirmed the new-document framing: NOT structurally blocked by the never-port-backprop decision. `ab_experiment.py`/`statistics.py` treat each arm's `params`/`structure` as fully opaque duck-typed objects; a Julia arm on a plain Lux.jl/Flux.jl MLP never touches FabricPC internals and is not a "port of `train_backprop.py`" in any sense. Real new code still needed (stats module + `ExperimentArm`/`ABExperiment` port + a small Lux arm) — moderate/large effort stands, "blocked" does not. 🔴 **CONFOUND, SCOPED TO A TRANSFORMER PC ARM ONLY (2026-07-14, `docs/decisions.md` §24) — does NOT block an MNIST-MLP arm.** A PC arm built on `transformer_lm()`'s own default (`eta_infer=0.1`) must not be compared without first checking that config's PC-relaxation conditioning — §24 measured the tiny diagnostic transformer graph as ~5.7× past its own stability limit at η=0.1 (12 steps buys ~0.6 of a stiff relaxation, not convergence), so a PC arm run there would benchmark a non-converged inference loop, not the algorithm. Measure or fix that arm's own `eta_infer`/`infer_steps` conditioning (same perturb-and-track + power-iteration method as §24) before running that specific comparison. **The MNIST-MLP config carries NO such confound** — §6/§24's own control measurement already found it contractive at its actual `eta_infer=0.1` (κ≈0.902) — so **a PC-vs-backprop comparison on MNIST-MLP is unconfounded and runnable today**, independent of this scope's transformer caveat and independent of J-01/J-04. |
