...
| J-02 | Compile full `train_step` | **✅ SHIPPED (2026-07-16) for the DENSE + plain-SGD lane — the full PC training step (E-step inference + M-step local weight update) now compiles into ONE XLA graph, verified correct.** `compile_train_step(structure, params, clamps; batch, lr, loop=true)` (ext/FabricPCReactantExt.jl) → callable `(params, init_state) -> GraphParams`. **How the "no flat weight-grad path" gap was closed:** built the M-step's positional twin in `src/jit_flat.jl` — `_flat_weight_grads` (per-node closed-form: Linear `dWₖ=insₖᵀ·dpre`, `db=Σdpre`; LinearResidual's in/"skip" split; Identity/Skip weightless), `flat_sgd_step` (settle → local grad → `w-=lr·dW`), `flat_train_step` (E+M eager reference), `jit_train_step_runner` (traceable entry), `graph_params_from_flat` (rebuild the Dict output). **Why NO Enzyme:** for the dense lane BOTH latent grads (`_flat_input_grads`) AND weight grads are closed-form local rules — all matmuls — so the whole step traces under the same `track_numbers=false` contract as `compile_inference` (J-09); Enzyme-under-Reactant (J-10 gate 2) is only the TransformerBlock weight backward, out of this scope. **VERIFIED (warm Reactant session, Reactant 0.2.274 / Enzyme 0.13.185):** eager `flat_train_step` == eager Dict `train_step` **bit-identical** (max\|Δw\|=max\|Δb\|=0.0, MLP + residual net — 2 new tests in `test/test_jit_flat.jl`, suite now 2381/2381); **compiled == eager 8.20e-8** (float32 reassociation, consistent with inference-only 1.49e-8 + the M-step matmuls) on both `loop=true` and `loop=false`; update is REAL (max\|Δ vs initial\|=0.168, so the M-step was NOT DCE'd — a DCE'd update would read ~lr·\|grad\|, orders larger, the #2647 hazard); 3-step composition stays bounded at 8.94e-8. Gate: `benchmark/compiled_train_step.jl` (PASS). **DELIBERATELY plain SGD, not AdamW — and that SIDESTEPS the frozen-`t` landmine below rather than needing Optimisers.jl:** SGD's `lr` is a compile-time constant with NO carried state, so `track_numbers=false` freezes nothing dynamic; it is also the faithful v0 port (`train.jl`: "v0 ships plain SGD"). **STILL OPEN (deferred, both real):** (1) **AdamW/momentum on the compiled lane** — THIS is where `docs/decisions.md` §25's landmine bites: the hand-rolled AdamW step counter is a plain Julia scalar, frozen at trace time under `@compile` unless tracked, so every compiled step would silently reapply step-1's bias correction. Use `Optimisers.jl` (regular dep, `OptimisersReactantExt` resolved, unused) functional opt-state on the compiled lane — retires the landmine AND closes the original audit's "generic optimizer injection" gap (upstream swaps `optax`). (2) **TransformerBlock compiled TRAINING** — needs `_flat_input_grads(::TransformerBlock)` (latent) + a flat transformer weight grad, both via Enzyme-under-Reactant (J-10 gate 2 proved the backward compiles+correct at 3.71e-7, but it is not yet wired into `src/`'s flat lane). |
...
| J-07 | Slot-name resolution + immutable state-Dict churn out of node forwards | OPEN — **QUANTIFIED (2026-07-14).** `Profile.Allocs` attribution of the same 6565 events: the dominant allocation *count* is `put_node` (`state_ops.jl:24`) at **1440 events**, each rebuilding the whole `Dict{String,NodeState}` + `NodeState` + `GraphState` (360 `Dict{String,NodeState}`, 320 `NodeState`, 242 `GraphState` allocated) on every single node update — the immutable-reconstruction Python-ism. The flat/positional path (`jit_flat.jl`, `Vector{NodeState}` indexed by position) already removes THIS Dict-rebuild, but keeps the J-06 boxing — so the two are separable and J-07's Dict-churn is fixed for free once a positional state replaces the `Dict{String,NodeState}` in the eager path. Related NEW finding: `forward_and_latent_grads`'s `in_degree==0` clamped-source branch (`linear.jl:129`) needlessly reallocates `copy(z_latent)` + `zero(error)` + `zero(pre_activation)` for the 256×784 clamped input **every step** — **48.2 MB, the single largest byte allocator** — a hoistable case §28's current opt (in_degree>0 only) does not yet cover. **SUB-ITEM CLOSED (2026-07-16, `6a26fe3`): the 48 MB clamped-source branch is now hoisted.** `_static_sources(clamps, structure)` collects clamped `in_degree==0` sources (which `_hoistable_nodes` excludes via `in_degree > 0 || continue`); `_run_inference_loop_opt` computes their state ONCE before the loop (`forward_and_latent_grads(..., true)` + `put_node`, bit-identical to the stock branch) and `_forward_value_and_grad_opt` skips them each step (`name in static && continue`). Their state is fully loop-invariant (z_latent clamped ⇒ z_mu=z_latent, error/energy/latent_grad constant; `zero_grads` still re-zeroes the never-read latent_grad). **MEASURED (256×784 MNIST-MLP, 20 steps, warm session, same state): opt path 5903 allocs/123.8 MB → 4933 allocs/47.4 MB (−76.4 MB, −970 allocs).** Bit-identical: opt-vs-stock z_latent max\|Δ\|=0.0 AND clamped-source z_mu max\|Δ\|=0.0 (Tier C/D dumps read the clamped state, so the full source state — not just latents — must match; verified). Full suite 2379/2379, 0 fail/error. **REMAINING (still OPEN): the DOMINANT part — `put_node`'s 1440-event Dict-rebuild — is the same refactor as the deferred in-place loop (J-06 addenda: replace `Dict{String,NodeState}` with a positional `Vector` in the eager path). Per §28 that eager-path Dict-churn is lower-leverage than J-02 and is NOT a J-02 blocker (the compiled lane uses `jit_flat.jl`'s positional path, which already avoids the Dict-rebuild). Deferred.** |
