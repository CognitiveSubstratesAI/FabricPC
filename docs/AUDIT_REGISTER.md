...
| ID | Pair | Status |
|----|------|--------|
| R-01 | train_autoregressive.jl vs .py (full) | **VERIFIED CLOSED.** Index-cast half (the remaining open question): read upstream `_generation_step`/`generate_autoregressive` (train_autoregressive.py:334-538) directly — the generation scan carry (`context_window`, `output_buffer`, sampled `next_token`) is int32 end-to-end for the EmbeddingNode path; `state_initializer.py:317-349`'s `_validate_clamp_dtypes` explicitly requires int (not float) clamps on source nodes, and `EmbeddingNode.forward` (transformer_v2.py:106-109) documents the same contract. Julia's `generate_autoregressive` (train_autoregressive.jl) keeps its loop-carried `context`/`generated` state as native `Int` between steps identically — the only float involvement is the transient per-step clamp force-cast (`state_ops.jl:45`, X-01) immediately rounded back in `EmbeddingNode`'s `_embed_lookup`, bit-exact for any realistic vocab size and non-compounding across steps (the outer carry never touches Float32). Same divergence X-01 already named and closed, not a new one — no case exists where this selects a different token than upstream for identical logits. Coverage caveat for future work: `test_transformer_lm.jl`'s "(c) greedy generation" testset is a self-consistency smoke test (shape/prefix/range/RNG-determinism), not a byte-level fixture against upstream JAX output like Tier A/B — would not catch a wrong-but-in-range index corruption, only a crash/NaN/out-of-range one. No fixture proposed for this as part of R-01. |
...
§21-23 written). R-01…R-05 all closed — R-05's graph_construction spot-check found and fixed a
real (dormant) duplicate-edge double-counting bug in the JIT flat path. C-04 (reclassified, not
blocking) remains as its inputs arrive.
