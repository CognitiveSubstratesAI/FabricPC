...
| A-03 | Adversarial passes predicting specific failures BEFORE execution (methodology evidence) | **Two instances on file; recording them because "our process predicts failures" is a claim that needs receipts, not testimonials.** (1) **F-01, muPC LayerNorm gate.** (2) **`_ad_param_grads` contract violation (2026-07-15, `df1d470`).** A reflective design pass over the C-01 port — run BEFORE any code was written or executed — read `ext/FabricPCZygoteExt.jl` and reported that `_ad_param_grads` captures the raw `inputs` **Dict** inside its differentiated closure, **contradicting that file's own header** ("no `Dict` enters the differentiated region"), while its sibling `_ad_latent_grads` complies; it flagged this as latent-but-live because every seam node is currently single-edge and ConvNode would be the first multi-edge one. It was right on both counts: changing `_concrete_inputs` from a hash `Dict` to an `OrderedDict` (B3, an independent fix) broke **precisely there** — `MethodError: getindex(::Dict{Any,Any})` across four Tier B testsets — because Zygote routes `getindex(::AbstractDict, k)` through dict-type-sensitive `grad_mut` machinery. The prediction named the file, the function, the mechanism, and the reason it was dormant, ahead of the run that proved it. Value: the bug was invisible to the full 579-test suite (it "worked" by accident with a plain `Dict`) and would have surfaced as an inexplicable conv failure much later. Fix = the same pass's prescribed remedy (hoist `SoA(inputs)` out of the closure). Corollary for readers: a passing suite is not evidence of a correct invariant — it is evidence that nothing has perturbed it yet. **(3) MaxPool tie-routing (2026-07-15) — the sharpest instance, because it is the one where the failure was invisible to the test we WOULD HAVE WRITTEN.** Building C-01 oracle-first, three design claims went to measurement and **exactly one survived**: (i) *tie-routing* — reasoning said window order was an implementation detail; the oracle said `lax.reduce_window`'s max VJP breaks ties in ROW-MAJOR/NHWC order (measured: partial-tie window `[[0,1],[1,0.5]]` → gradient lands on `(0,1)`), while our im2col patch layout is COLUMN-MAJOR and would have picked `(1,0)` and shipped wrong; (ii) *B5 `pre_activation`* — reasoning called it a rounding-level bookkeeping nit; the oracle said `max|pre − z_mu| = 4.81` for a ReLU conv, i.e. gross; (iii) *"square kernels could hide a transposition"* — reasoning worried; measurement dissolved it (the fixture weights are `jax.random.normal`, so asymmetry already broke the mask — the distinct extents we added buy a better failure MODE, loud shape error instead of numeric miss, not a new detection). **The generalisable claim is NOT "measurement verifies code" — it is "measurement constructs the discriminating case that reasoning does not know it needs."** The tie-routing bug proves it: the natural test to write is an all-tied window, and an all-tied window passes under BOTH orders (both start at `(0,0)`), so our own testing instinct would have papered over it. Only forcing the oracle to answer a PARTIAL-tie case — which we had no a priori reason to construct — discriminated. Recorded at this specificity deliberately: the ecosystem's default prior on "my port is conformance-tested" is skepticism, and skepticism is answered with receipts (file, function, mechanism, measured value, and which claims LOST), not assurances. |
...
| ID | Sev | Item | Status |
|----|-----|------|--------|
| C-01 | HIGH | ConvNode (1D/2D/3D) + pooling + windowed-shape validation | **IN PROGRESS (2026-07-15).** Prerequisites LANDED (`df1d470`): C-08 ND fan-in, rank-4 params through the optimizers (11 hardcoded `Dict{String,Matrix{Float32}}` sites — a conv graph would have forwarded fine and died at the first weight update), the `compute_pre_and_mu` seam hook (B5), and a latent seam fold-order bug (below). Architecture confirmed by direct read: upstream ConvNode/`_PoolBase` implement ONLY `forward` and inherit `NodeBase`'s `jax.value_and_grad` gradients (`base.py:507-551`) — which maps 1:1 onto this port's `compute_mu`/`energy_kernel` Enzyme/Zygote seam, so each node needs only a `compute_mu`. Rank-4 plumbing needed NO change (`state_initializer.jl:30` splats `(batch, shape...)`, GaussianEnergy reduces over all non-batch dims, TransformerBlock already runs rank-3). **ORACLE BUILT BEFORE THE KERNEL** (`scripts/generate_tier_conv_fixtures.py` → `test/conformance/fixtures/tier_conv.npz`, 121 arrays, upstream jax 0.10.2): forward AND both autodiff grad paths for an overlap-heavy stride-1 backward, a 3-in-edge fold-order case, ReLU/strided/VALID, 1D/3D, and pooling. Deliberate sequencing — a forward that matches while the backward folds differently is the invisible-divergence pattern the whole conformance effort exists to catch. Chosen design (reflective bake-off, 3 proposals judged): hand-rolled im2col, **zero new deps**; padding is a TYPE not a string (Enzyme provably rejects `uppercase(::String)` inside the differentiated kernel); `cld`/`fld` **never `÷`** (Python `//` floors, Julia `÷` truncates — a *fail-open*: `n<k` raises upstream but would silently validate here). REMAINING: the im2col kernel, ConvNode, MaxPool/AvgPool, wiring, `test_tier_conv.jl` + structural tests, adversarial audit. **🔴 "Fold order" — same word, DIFFERENT stakes than `df1d470`/`5663c3f`. Record this or it reads as an inconsistency ("you made fold order a headline for the seam; why is conv's scatter ungated?").** The two are not the same class: (a) **the seam fold (B3) is two implementations of the SAME computation** — a node's `forward` and its differentiated lanes — disagreeing. The operand order there is *ours to choose*, so bit-identity is achievable and therefore REQUIRED; any divergence is a defect, and it is asserted bit-exactly (`forward`'s `z_mu` ≡ the seam's `energy_kernel` `z_mu`; multi-edge fold ≡ `in_edges`). (b) **conv's col2im scatter is two DIFFERENT algorithms** — our AD reversing an im2col gather→GEMM vs XLA's fused conv VJP — computing the same mathematical quantity. Bit-identity is unachievable *by construction*: they reassociate differently at ~1e-7 for Float32. So rtol 1e-5 is the only meaningful gate, and it is the CORRECT one — it catches every structural bug (index map, kernel flip, pad, transposition, tie-routing), all of which land far above 1e-5, while tolerating legitimate reassociation. **Consequence, stated honestly: conv's scatter fold order is NOT a gated correctness axis, deliberately.** The defensible claim is "order changes only the last ULP here, below the tolerance that matters", NOT "fold order is tested". A ULP-tight assert on the overlap backward was considered and REJECTED: order-wrong scatter (~1e-7) and legitimate im2col-vs-XLA reassociation (~1e-7) are the same magnitude, so such a gate would fail on correct code as often as wrong — a flaky test dressed as a rigorous one. Bit-identity is a valid instrument only where both sides execute the same operation order; rtol is the instrument everywhere else. |
| C-02 | HIGH | ResNet-18/CIFAR-10 demo | BLOCKED (C-01) |
| C-14 | MED | Enzyme lane: hoist `SoA(inputs)` out of `_ad_param_grads`'s closure (uniform with the Zygote lane) | **DEFERRED — WITH AN EXPLICIT TRIGGER, because the deferral has now been correct three times and that is exactly how an item rots into permanently-deferred.** 🔔 **TRIGGER: the moment ConvNode lands, test this. Do not re-defer.** Why it is *currently* correct to defer: `ext/FabricPCEnzymeExt.jl:58-62` passes `Const(inputs)` — Enzyme does not differentiate `Const` args, so `df1d470`'s `Dict`→`OrderedDict` change (B3) is genuinely INERT there, and `.warm/zygote_env` has no Enzyme, so touching it would be an unverifiable blind edit (`feedback_no_coldstart_testing` / don't change what you cannot run). Why the trigger is exactly ConvNode: the Zygote lane's identical bug was dormant *only* because every seam node is single-edge; **ConvNode/MaxPool/AvgPool are the first `is_multi_input` seam nodes and the first to ITERATE `inputs` on the tape** — and `src/core/types.jl:47-54` records that a `Dict`'s i64 hash internals are what trips Enzyme's `addToDiffe`. So ConvNode simultaneously (a) makes the hazard live and (b) makes it testable. Per the C-01 plan: if the Enzyme lane aborts on ConvNode, the Zygote lane still closes C-01 and the Enzyme gap is recorded HERE as a gap — it does not get absorbed into a conformance claim. |
