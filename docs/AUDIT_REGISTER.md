...
|----|------|--------|--------|
| A-01 | Re-sync Python reference to upstream HEAD | **RE-OPENED 2026-07-15 — upstream moved, and the SHA is the only real marker (A-01's own original lesson).** State as of today: **fixtures = upstream `5514c91`; upstream HEAD = `b6f64ad`.** The one intervening commit changes the `forward()` contract twice (drops `NodeState.pre_activation`; returns a bare `NodeState` instead of `(total_energy, NodeState)`) — see C-15 for the impact analysis and C-16 for the adopt/diverge decision. **The oracle's source moved under us mid-session**: `~/PRIMUS/dev-zone/FabricPC` is a SYMLINK to `~/JuliaAGI/dev-zone/FabricPC`, which is what `.venv-fixtures` imports via its EDITABLE install — so a `git pull` in the checkout silently re-points the fixture generator at new code while the committed `.npz` still holds the old numbers. That asymmetry (arrays frozen, generator live) is the whole hazard, and it is why the SHA is now pinned IN the generator rather than described as "the local checkout". **What is NOT affected, verified by reading the diff rather than the release summary:** `core/energy.py` is unchanged (`state.energy` was always per-sample; only the summing moved to the callsites), so no fixture's CONTENT is invalidated; and `architectural_refactor_proposal.md` is a 2026-05-04 review doc archived into `docs/`, not a new structural change. C-01's measured numerics are untouched. |
...
| C-15 | HIGH | Upstream `forward()` contract changed (b6f64ad): `pre_activation` dropped from `NodeState`; `forward()` returns `NodeState` only | **OPEN — ORACLE DRIFT, not a numerics problem. Recorded the day it appeared (2026-07-15); deliberately NOT fixed under close-everything pressure.** Upstream moved 5514c91 -> b6f64ad: *Remove `pre_activation` from `NodeState`; `forward()` returns `NodeState` only (#25)*. **Read the diff, not the summary — two things people will assume changed did NOT:** (a) `core/energy.py` is UNCHANGED, so `state.energy` was ALWAYS per-sample; only the SUMMING moved out of `forward()` into the autodiff callsites (`total_energy = jnp.sum(new_s.energy)` inside `energy_fn`). So our fixtures' `energy` arrays are the same quantity and are NOT invalidated. (b) `architectural_refactor_proposal.md` is a HISTORICAL review doc (dated 2026-05-04, branch `clinfra`), archived into docs — not a new structural change. The real delta is ONE commit. **What IS affected:** (1) our `NodeState` now carries a field upstream deleted (`pre_activation`) — a pytree-structure divergence, and our fixtures record `*_fwd_pre_activation` upstream no longer produces; (2) our Julia `forward` still returns `(sum(energy), state)` where upstream returns a bare state; (3) **`scripts/generate_tier_conv_fixtures.py` will SILENTLY CORRUPT if re-run against HEAD** — its `_, fwd = forward(...)` unpacks a tuple that no longer exists, and because `NodeState` is a NamedTuple that unpack does not raise, it binds `_`/`fwd` to the first two FIELDS. Fail-open. The script now carries a loud SHA-pinned warning. **Not urgent, and the reason matters:** C-01's numerics are gated at ~1e-7 against fixtures that were valid when generated; nothing measured changes. This is the oracle drifting away from the port, which is a regeneration-time hazard, not a correctness one. Per `feedback_verify_oracle_against_upstream_not_assume_canonical`, the next regeneration is a deliberate oracle-first task (adapt the generator, decide C-16, re-pin the SHA), NOT a re-run. |
| C-16 | MED | Adopt upstream's `pre_activation` removal, or diverge deliberately | **OPEN — a DECISION, not a bug.** Upstream reclassified `pre_activation` as a transient forward intermediate (recomputed each forward, consumed once by `LinearExplicitGrad`) rather than durable `NodeState`, citing per-node/per-timestep memory and the hidden constraint every node implementer must remember to populate. Following them removes a pytree-structure divergence and is the cleaner design; diverging keeps our B5 bookkeeping. **B5 is NOT invalidated either way** — the oracle measured `max|pre - z_mu| = 4.81` for a ReLU conv, so the VALUE is real; upstream only changed WHERE it lives (threaded locally via `_forward_with_preact` rather than stored). **Already-verified corollary:** upstream's own review caught a bug in their FIRST attempt — a generic pre-activation helper carries a silent Linear-only contract (`LinearResidual` sums `:skip` AFTER activation, `StorkeyHopfield` blends by strength, `Transformer` has no classical pre-activation), so wrapping any of them yields a wrong `f'(z)`. **Our `compute_pre_and_mu` does not have that bug**: the generic default is `(z_mu, z_mu)` — it reports z_mu as the pre and invents nothing — and ONLY ConvNode/PoolNode, which genuinely have a distinct pre-activation, override it. Convergent with upstream's fix, arrived at independently (the constraint that forced it: keeping every existing seam node byte-identical). |
